...
def build_reference_square(start_xy: np.ndarray, side_length: float, rotation_deg: float, n_points: int) -> np.ndarray:
    n_points = max(2, int(n_points))
    perimeter = 4.0 * side_length
    distances = np.linspace(0.0, perimeter, n_points)

    square_local = np.zeros((n_points, 2), dtype=float)
    for idx, d in enumerate(distances):
        if d <= side_length:
            square_local[idx] = [d, 0.0]
        elif d <= 2 * side_length:
            square_local[idx] = [side_length, d - side_length]
        elif d <= 3 * side_length:
            square_local[idx] = [side_length - (d - 2 * side_length), side_length]
        else:
            square_local[idx] = [0.0, side_length - (d - 3 * side_length)]
    ...
    return (square_local @ rot.T) + start_xy



def compute_reference_square_metrics(coords: np.ndarray, start_idx: int, end_idx: int, side_length: float, rotation_deg: float):
    end_idx = min(end_idx, len(coords) - 1)
    start_idx = max(0, min(start_idx, end_idx - 1))
    measured = coords[start_idx:end_idx + 1]
    if len(measured) < 2:
        return None

    reference_xy = build_reference_square(measured[0, :2], side_length, rotation_deg, len(measured))
    ...
    reference_length = 4.0 * side_length
...
def make_2d_plot(
    ...
    if square_metrics is not None:
        ...
        if view == "Top-down (X-Y)":
            ax.plot(ref_xy[:, 0], ref_xy[:, 1], linestyle="--", linewidth=2, label="Reference square")
            ax.plot(seg[:, 0], seg[:, 1], linewidth=2.5, label="ATE segment")
            ax.scatter(ref_xy[0, 0], ref_xy[0, 1], marker="D", s=50)
            ax.legend()
        elif view == "Front (X-Z)":
            ax.plot(seg[:, 0], seg[:, 2], linewidth=2.5, label="ATE segment")
            ax.legend()
        elif view == "Side (Y-Z)":
            ax.plot(seg[:, 1], seg[:, 2], linewidth=2.5, label="ATE segment")
            ax.legend()
...
def make_3d_plot(
    ...
    if square_metrics is not None:
        ...
        ax.plot(ref_xy[:, 0], ref_xy[:, 1], ref_z, linestyle="--", linewidth=2, label="Reference square")
        ax.plot(seg[:, 0], seg[:, 1], seg[:, 2], linewidth=2.5, label="ATE segment")
...
with st.sidebar:
    ...
    st.subheader("Reference square ATE")
    enable_square_ate = st.checkbox("Enable square overlay ATE", value=False)
    square_metrics = None
    if enable_square_ate:
        ate_start_idx = st.slider("ATE segment start sample", min_value=0, max_value=n_points - 2, value=0)
        ate_end_idx = st.slider("ATE segment end sample", min_value=ate_start_idx + 1, max_value=n_points - 1, value=n_points - 1)
        square_side = st.number_input("Reference square side length (m)", min_value=0.01, value=1.00, step=0.05, format="%.2f")
        square_rotation_deg = st.number_input("Square rotation on map (deg)", value=0.0, step=5.0, format="%.1f")
        square_metrics = compute_reference_square_metrics(plot_filtered, ate_start_idx, ate_end_idx, square_side, square_rotation_deg)
...
if square_metrics is not None:
    st.subheader("Reference square error")
    ...
    s4.metric("Length error", f"{square_metrics['measured_length'] - square_metrics['reference_length']:.3f} m")
    st.caption(
        "This is a reference-path RMSE against an ideal square drawn in the X-Y plane and anchored at the selected segment start. It is useful for demo evaluation, but it is not formal benchmark ATE unless you have matched ground-truth poses."
    )
...
st.write(
    "The filtered path is computed with a moving-average smoother, and the filtered path length ignores tiny frame-to-frame motions below the selected threshold. You can also compare a chosen path segment against an ideal square overlay to get a practical XY reference-path RMSE."
)
...
st.caption(
    "Tip: increase the smoothing window or minimum step threshold if VIO jitter is making the path look too zig-zaggy. Use the square overlay only when the test path is actually intended to approximate a square."
)
